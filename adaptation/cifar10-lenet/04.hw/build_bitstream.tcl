...
# build_bitstream.tcl
#
# Orchestrates the full Vivado flow for v0.2 (CIFAR-10 CNN):
#   1. Create Vivado project targeting the AUP-ZU3 board
#   2. Register the hls4ml-generated IP repository
#   3. Source cifar10_bd.tcl (block design adapted from ICTP)
...
set proj_name    "cifar10_lenet"
set proj_dir     [file join $script_dir vivado_prj]
set ip_repo_dir  [file normalize [file join $script_dir .. 03.hls myproject_accel_prj solution1 impl ip]]
set bd_script    [file join $script_dir cifar10_bd.tcl]
set design_name  "machineLearning_bd"
set part         "xczu3eg-sfvc784-2-e"
set board_part   "realdigital.org:aup-zu3-8gb:part0:1.0"
set xsa_name     "bd_wrapper_cifar10.xsa"
...
#
# Memory throttling: this host has 15 GB RAM. The big win was forcing
# the BD to global synth (synth_checkpoint_mode None above), which
# eliminated the 9 parallel OOC sub-runs. We additionally cap the
# internal thread pool so synth_design itself doesn't burn all CPUs.
set_param general.maxThreads 2

puts "INFO: Launching synthesis (this can take 30-60 min with throttled parallelism)"
