...
Output: output/myproject_wbc64_v05_prj/ (firmware + weights for the HLS stage)
"""
import os
import warnings
...
OUT_DIR = os.path.join(HERE, "output", "myproject_wbc64_v05_prj")
PART = "xczu3eg-sfvc784-2-e"
SIZE = 64

REUSE_PLAN = {
    "conv1":  {"Strategy": "Resource", "ReuseFactor": 27},
    "conv2":  {"Strategy": "Resource", "ReuseFactor": 216},
    "conv3":  {"Strategy": "Resource", "ReuseFactor": 288},
    "output": {"Strategy": "Resource", "ReuseFactor": 12},
}
...
def main():
    ...
    np.save(os.path.join(HERE, "ref_pred_sw64_v05.npy"), y_sw)
