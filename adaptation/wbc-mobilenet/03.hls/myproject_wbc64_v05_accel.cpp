#include "myproject_wbc64_v05_accel.h"
#include "myproject.h"

// AXI-Stream + AXI-Lite wrapper around the hls4ml wider CNN (v0.5) for
// BloodMNIST. See the header for the protocol. Identical wrapper logic to
// v0.4 — the only difference is the larger internal hls4ml core.
//
// Input:  WBC_NSAMPLES = 12288 beats of 32 bits each. Each beat carries
//         one fixed<24,12> in its low 24 bits. Beats are ordered row-major
//         over (H, W, C) with C running fastest (R, G, B of one pixel,
//         then the next pixel along the row).
// Output: WBC_NCLASSES = 8 beats, one fixed<24,12> class score each.
//         Last beat has TLAST = 1.

void myproject_wbc64_v05_accel(
