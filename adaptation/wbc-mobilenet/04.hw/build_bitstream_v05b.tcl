...
#   3. Source wbc64_v05b_bd.tcl (block design adapted from ICTP)
...
set proj_name    "wbc64_v05b_mobilenet"
set proj_dir     [file join $script_dir vivado_prj_v05b]
set ip_repo_dir  [file normalize [file join $script_dir .. 03.hls myproject_accel64_v05b_prj solution1 impl ip]]
set bd_script    [file join $script_dir wbc64_v05b_bd.tcl]
set design_name  "machineLearning_bd"
set part         "xczu3eg-sfvc784-2-e"
set board_part   "realdigital.org:aup-zu3-8gb:part0:1.0"
set xsa_name     "bd_wrapper_wbc64_v05b.xsa"
...
#
# v0.5b: throttled HARD so the host stays usable for other work (LaTeX) in
# parallel. We drop to 2 threads / 1 job — the lightest setting (same as the
# v0.3 build on a 15 GB host). v0.5 at 4 threads / 2 jobs peaked ~8 GB; at
# 2 threads / 1 job the peak is lower and CPU pressure is minimal. It just
# takes longer, which is fine since it runs unattended in the background.
# The BD stays in global synth mode (synth_checkpoint_mode None above) so no
# parallel OOC sub-runs spawn.
set_param general.maxThreads 2

puts "INFO: Launching synthesis (throttled: 2 threads / 1 job, host stays free)"
